...
public class Result {
    ...
    public String getTripResult() {
        ...
        for (int i = 0; i < point.length; i++) {
            returnArrayJson.append("{\"name\":\"" +
                    (point[i].getName() == null ? "" : point[i].getName().replace("\"", "\\\"")) +
                    "\",\"amenity\":\"" + point[i].getAmenity() +
                    "\",\"latitude\":\"" + point[i].getLatitude() +
                    "\",\"longitude\":\"" + point[i].getLongitude() + "\"}");
            if (i < point.length - 1)
